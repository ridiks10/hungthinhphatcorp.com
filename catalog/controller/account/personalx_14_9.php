<?php
class ControllerAccountPersonalx extends Controller {
	private $error = array();
...
$this->document->addScript('catalog/view/javascript/personal/tree.min.js');
		...
		if (file_exists(DIR_TEMPLATE . $this -> config -> get('config_template') . '/template/account/personalx.tpl')) {
			$this -> response -> setOutput($this -> load -> view($this -> config -> get('config_template') . '/template/account/personalx.tpl', $data));
		} else {
			$this -> response -> setOutput($this -> load -> view('default/template/account/personalx.tpl', $data));
		}
	}
