...
class ModelAccountAuto extends Model {
	...
	public function getDay_cycle($day, $cycle){
		$query = $this -> db -> query("
			SELECT *
			FROM ". DB_PREFIX . "customer
			WHERE date_register_tree <= DATE_ADD(NOW(),INTERVAL - ".$day." DAY) AND active_tree = 1 AND cycle = ".$cycle."
		");
		
		return $query -> rows;
	}
	public function updateCycle($pd_id){
		$this -> db -> query("UPDATE " . DB_PREFIX . "customer SET 
			cycle = cycle + 1 
			WHERE customer_id = '".$pd_id."'
		");
	...
	public function auto_find_update_status_ml_no_active_1btc(){
		echo "UPDATE ". DB_PREFIX . "customer_ml SET
			`status` = '-1'
			WHERE customer_id IN (SELECT customer_id FROM ". DB_PREFIX . "customer WHERE active_tree = 0 AND date_register_tree <= NOW() )";echo '<br>';
		$query = $this -> db -> query("
			UPDATE ". DB_PREFIX . "customer_ml SET
			`status` = '-1'
			WHERE customer_id IN (SELECT customer_id FROM ". DB_PREFIX . "customer WHERE active_tree = 0 AND date_register_tree <= NOW() ) ");
	}
}