...
class ModelAccountCustomer extends Model {
	...
	public function getInfoUsers_binary($id_id){
		...
		$query = $this->db->query("select u.*,ml.level,ml.p_binary as p_binary_ml, u.firstname as account_holder,
			...
			ON l.id = ml.level Where ml.customer_id = " . $id_id);
		$return  = $query->row;
		return $return;
	}
	...
	public function getPNode($customer_id){
		$query = $this -> db -> query("
			...
			(SELECT customer_id FROM sm_customer WHERE p_node = ".$customer_id.") AND status = 1 GROUP BY customer_id");
		return $query -> rows;
	}
	public function getCustomer_By_PNode_Package($customer_id,$packae){
		$query = $this -> db -> query("
			SELECT * FROM sm_customer_provide_donation where customer_id in 
			(SELECT customer_id FROM sm_customer WHERE package = '".$packae."' AND p_node = ".$customer_id.") AND status = 1 GROUP BY customer_id");
		return $query -> rows;
	}
	...
	public function getPDByTranferID($transacion_id){
		$query = $this -> db -> query("
			...
			WHERE transfer_code = '".$transacion_id."'
		");
		return $query -> row;
	}
 ...
 as date_add_login,ip.ip
	...
	public function getAllCustomer() {
		$query = $this -> db -> query("SELECT ct.name as country_name,c.username FROM " . DB_PREFIX . "customer c JOIN sm_country ct on c.country_id = ct.country_id ORDER BY c.date_added DESC LIMIT 50");
		return $query -> rows;
	}
	...
	function getInfoUsers($id_ids) {
		...
		$query = $this -> db -> query("select u.*,mlm.level, ml.p_binary as p_binary_ml, ml.status as status_ml, l.name_vn as level_member from " . DB_PREFIX . "customer as u Left Join " . DB_PREFIX . "customer_ml as mlm ON mlm.customer_id = u.customer_id  Left Join " . DB_PREFIX . "member_level as l ON l.id = mlm.level  Where u.customer_id IN " . $array_id);
		if (!is_array($id_ids)) {
			$return = $query -> row;
		} else {
			$return = $query -> rows;
	...
	function getCount_ID_BinaryTreeCustom($id_user) {
		$listId = '';
		//echo "select customer_id from " . DB_PREFIX . "customer_ml where p_binary = " . (int)$id_user."";echo '<br>';
	...
	function getCount_Floor($id_user) {
		if ($id_user) {
			echo "SELECT `left`,`right` FROM " . DB_PREFIX . "customer_ml 
			where customer_id IN (".$id_user.") ";echo '<br>';
			//echo "select customer_id from " . DB_PREFIX . "customer_ml where p_binary = " . (int)$id_user."";echo '<br>';
			$query = $this -> db -> query("SELECT `left`,`right` FROM " . DB_PREFIX . "customer_ml 
				where customer_id IN (".$id_user.") ");
			return $query -> rows;
			}
	...
	public function update_package($id_customer,$package){
		$query =  $this -> db -> query("
			UPDATE " . DB_PREFIX . "customer SET
			package = '".$package."'
			WHERE customer_id = '" . (int)$id_customer. "'");
	...
	public function update_active_tree($id_customer , $active){
		$query =  $this -> db -> query("
			UPDATE " . DB_PREFIX . "customer SET
			active_tree = '".$active."'
			WHERE customer_id = '" . (int)$id_customer. "'");
	...
	public function getPDConfirm_ByTranfer($tranfer, $customer_id){
		$query = $this -> db -> query("
			SELECT *
			FROM ". DB_PREFIX . "customer_invoice_pd
			WHERE transfer_id = '".$this->db->escape($tranfer)."' AND customer_id = ".$customer_id."
		");
		return $query -> row;
	}
	public function getPD_By_PD_Numner($pd_number){
		$query = $this -> db -> query("
			SELECT status
			FROM ". DB_PREFIX ."customer_provide_donation
	...
	public function getAllPDByTranferID($pd_id){
		$query = $this -> db -> query("
			SELECT *
			FROM ". DB_PREFIX . "customer_provide_donation
			WHERE pd_number = '".$this->db->escape($pd_id)."'
		");
		return $query -> row;
	}
	...
	{
		echo "SELECT * FROM sm_customer_ml WHERE customer_id = ".$customer_id.""; echo '<br>';
	    $query = $this -> db -> query("SELECT * FROM sm_customer_ml WHERE customer_id = ".$customer_id."");
		$array = $query -> row;
	    $count = 0;
	    if(!empty($array['left']))
	...
	public function get_package(){
		...
		return $query -> row['package'];
	}
	public function get_date_register_tree(){
		$query = $this -> db -> query("SELECT date_register_tree 
			FROM " . DB_PREFIX . "customer 
			WHERE date_register_tree >= NOW() 
			AND customer_id = '" . $this -> session -> data['customer_id'] . "'");

		return $query -> row;
	}
	public function update_date_register_tree($customer_id){
	...
	public function getall_user()
	{
		$query = $this -> db -> query("
			...
			FROM ". DB_PREFIX . "customer A INNER JOIN ". DB_PREFIX . "country B ON A.country_id = B.country_id
		");
		return $query -> rows;
	}
