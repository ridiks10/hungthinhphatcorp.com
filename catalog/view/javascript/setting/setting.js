$( document ).ready(function() {
  $('#reset_passwdTran').on('click', function(){
    ...
    funSetting.loadingAccount(function(result) {
        _.has(result, 'username') && $('#UserName span').html(result.username);
        _.has(result, 'email') && $('#Email span').html(result.email);
        _.has(result, 'telephone') && $('#Phone span').html(result.telephone);
        _.has(result, 'address_cmnd') && $('#Address span').html(result.address_cmnd);
        _.has(result, 'firstname') && $('#fullname span').html(result.firstname);
        _.has(result, 'countryname') && $('#Country span').html(result.countryname);
        _.has(result, 'ip') && $('#LastIP span').html(result.ip);
        _.has(result, 'date_add_login') && $('#date_add_login span').html(result.date_add_login);
        _.has(result, 'date_added') && $('#Date span').html(result.date_added);
         _.has(result, 'wallet') && $('#wallet span').html(result.wallet);
        _.has(result, 'wallet') && $('#qrcode span').html('<img src="https://chart.googleapis.com/chart?chs=200x200&amp;chld=L|1&amp;cht=qr&amp;chl=bitcoin:'+result.wallet+'"/>');
